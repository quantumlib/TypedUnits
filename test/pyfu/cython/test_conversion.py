...
def test_conversion_times():
    ...
    # All elements.
    p = u.conversion_times(conv(factor=3.14, numer=5, denom=12, exp10=2),
...
def test_conversion_div():
    ...
    assert u.conversion_div(
        conv(numer=2), conv(numer=3)) == conv(numer=2, denom=3)
    assert u.conversion_div(
        conv(denom=2), conv(denom=3)) == conv(denom=2, numer=3)
    assert u.conversion_div(conv(exp10=2), conv(exp10=3)) == conv(exp10=-1)

    # All elements.
    d = u.conversion_div(conv(factor=3.14, numer=5, denom=12, exp10=2),
                         conv(factor=2.71, numer=2, denom=15, exp10=-5))
...
def test_conversion_raise_to():
    ...
    assert u.conversion_raise_to(
        conv(factor=4), frac(-2)) == conv(factor=0.0625)
    assert u.conversion_raise_to(
        conv(factor=3), frac(denom=2)) == conv(factor=1.7320508075688772)
    assert u.conversion_raise_to(
        conv(factor=3), frac(5, 9)) == conv(factor=1.8410575470987482)
    assert u.conversion_raise_to(
        conv(factor=3), frac(-5, 9)) == conv(factor=0.5431660740729487)
    ...
    # Lose precision.
    assert u.conversion_raise_to(
        conv(numer=3), frac(denom=2)) == conv(factor=1.7320508075688772)
    assert u.conversion_raise_to(
        conv(numer=512), frac(5, 11)) == conv(factor=17.040657431039403)
    assert u.conversion_raise_to(
        conv(numer=512), frac(-5, 11)) == conv(factor=0.058683181916356644)
    ...
    # Lose precision.
    assert u.conversion_raise_to(
        conv(denom=3), frac(denom=2)) == conv(factor=0.5773502691896258)
    assert u.conversion_raise_to(
        conv(denom=512), frac(5, 11)) == conv(factor=0.058683181916356644)
    assert u.conversion_raise_to(
        conv(denom=512), frac(-5, 11)) == conv(factor=17.040657431039403)
    ...
    # Lose precision.
    assert u.conversion_raise_to(
        conv(exp10=3), frac(denom=2)) == conv(factor=31.622776601683793)
    assert u.conversion_raise_to(
        conv(exp10=18), frac(5, 11)) == conv(factor=151991108.2952933)
    assert u.conversion_raise_to(
        conv(exp10=18), frac(-5, 11)) == conv(factor=6.579332246575682e-9)
